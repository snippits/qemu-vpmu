...
#pragma message("VPMU SET: 64 bits Not supported!!")
        ...
        if (likely(flag_tracing)) {
            ...
            if (unlikely(current_pid == 0)) {VPMU.enabled = 0; return ;}
            //Loop through pid list to find if hit
            for (int i = 0; traced_pid[i] != 0; i++) {
                if (current_pid == traced_pid[i]) {
                    flag_hit = 1;
            ...
            if (likely(flag_hit)) VPMU.enabled = 1;
            else VPMU.enabled = 0;
        }
    }
#endif
    ...
    if (likely(env && VPMU.enabled)) {
        ...
        if (vpmu_model_has(VPMU_ICACHE_SIM, VPMU)) {
            if (vpmu_model_has(VPMU_JIT_MODEL_SELECT, VPMU)) {
                ...
                int distance = VPMU.total_tb_visit_count - extra_tb_info->modelsel.last_visit;
                if (0 < distance && distance < 100) {
                    VPMU.hot_icache_count += extra_tb_info->modelsel.num_of_cacheblks;
                    extra_tb_info->modelsel.last_visit++;

                    VPMU.hot_tb_visit_count++;
                } else {
                    extra_tb_info->modelsel.last_visit = VPMU.total_tb_visit_count;
                    cache_ref(PROCESSOR_CPU, 0, extra_tb_info->start_addr,
                            CACHE_PACKET_INSTRN, extra_tb_info->counters.size_bytes);
...
/* FIXME move to ARM CPU model
            if (likely(VPMU.cpu_model.dual_issue)) {
                ...
                for (i = 0; i < ARM_VFP_INSTRUCTION_TOTAL_COUNTS; i++) {
                    VPMU.VFP_count[i] += extra_tb_info->vfp_count[i];
                }
#endif
