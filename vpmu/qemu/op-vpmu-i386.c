...
void HELPER(vpmu_accumulate_tb_info)(CPUX86State *env, void *opaque)
{
    ...
    // cpl : CPU-Privilege-Level = User(ring3) / Supervisor(ring0)
...
{
    //CPUState *cs = CPU(ENV_GET_CPU(env));
    // mode = CPU-Privilege-Level = User(ring3) / Supervisor(ring0)
    int mode = env->hflags & (3); 
    ...
    if (likely(env && VPMU.enabled)) {
        switch(mode){
            ...
            case 3:
                break;
            default:
                CONSOLE_LOG("unhandled privilege mode : %d\n", mode);
        }
    }
