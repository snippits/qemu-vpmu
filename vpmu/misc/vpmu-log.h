...
extern FILE* vpmu_console_log_fd;

#ifdef CONFIG_VPMU_DEBUG_MSG
#define DBG(str, ...)                                                                    \
    do {                                                                                 \
        fprintf(vpmu_console_log_fd, str, ##__VA_ARGS__);                                \
        fflush(vpmu_console_log_fd);                                                     \
    } while (0)
#else
#define DBG(str, ...)                                                                    \
    {                                                                                    \
...
#define CONSOLE_LOG(str, ...) fprintf(vpmu_console_log_fd, str, ##__VA_ARGS__)
