...
#define _CR(_VADDR_, cb)                                                                 \
    event_tracer.get_kernel().register_return_callback(                                  \
      _VADDR_, [](void* env, KernelState& state) { cb });
    ...
    _CB(ET_KERNEL_CONTEXT_SWITCH,
        {
            ...
            if (et_find_traced_pid(state.current_pid)) {
                et_set_process_cpu_state(state.current_pid, env);
                VPMU.enabled = true;
            } else {
    ...
    _CB(ET_KERNEL_EXIT,
        {
            // Do nothing if the value is not initialized
            if (g_linux_offset.task_struct.pid == 0) return;
            et_remove_process(state.current_pid);
            return;
    ...
    _CB(ET_KERNEL_WAKE_NEW_TASK,
        {
            ...
            if (state.current_pid != 0 && et_find_traced_pid(state.current_pid)) {
                et_attach_to_parent_pid(state.current_pid, target_pid);
            }
        },
        1);
    ...
    _CB(ET_KERNEL_MMAP,
        {
            ...
            et_parse_dentry_path(
              env, dentry_addr, fullpath, &position, sizeof(fullpath), 64);
            ...
            */
            ...
            if (state.current_pid == state.exec_event_pid && (mode & VM_READ)) {
                // Mapping executable page for main program
                if (et_find_program_in_list(state.bash_path)) {
                    et_add_new_process(fullpath, state.bash_path, state.current_pid);
                ...
                et_add_process_mapped_file(state.current_pid, fullpath, mode);
        ...
        if (state.mmap_update_flag) {
            ...
            */
            ...
            et_update_last_mmaped_binary(
              state.current_pid, et_get_ret_value(env), state.last_mmap_len);
        }
    });
